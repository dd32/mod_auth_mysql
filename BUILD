BUILDING and INSTALLING mod_auth_mysql using make
=================================================

If you are using apache2 you can easily build and install the module,
using the supplied Makefile.

To build mod_auth_mysql as a DSO:
	make

If apxs2 isn't present on your system, but you have apxs, try:
...
BUILDING mod_auth_mysql by hand
===============================
...
apxs -c -lmysqlclient -lm -lz -lcrypto mod_auth_mysql.c crypt_private.c
...
Note: In some Apache 2.x setups, apxs is called apxs2
...
apxs -c -L/usr/lib/mysql -I/usr/include/mysql -lmysqlclient -lm -lz -lcrypto mod_auth_mysql.c crypt_private.c
...
AuthMySQLPwEncryption		ENCRYPTION (3)	"none", "crypt", "scrambled", "md5", "aes", "sha1", "phpass"
